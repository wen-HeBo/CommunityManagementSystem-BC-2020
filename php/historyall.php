<?php 
	...
	// 首次查询，只需要得到记录数量即可
	if($pageIndex==0){
		$query = "select count(*) from history where UID='$oldid'";
		// 连接数据库
		...
		$query = "select * from history,food where UID='$oldid' and FID=OHID ORDER BY OHDATE limit ".$startRowNum.",".$numOfRows;
		// 连接数据库
		$conn = db_connection(DB_HOST,DB_USER,DB_PASSWORD,DB_NAME);
		// 执行查询操作
		$result = $conn->query($query);
